...
if (Meteor.isServer) {
  ...
  Meteor.startup(function () {
    ...
    var workers = _queue.processJobs(
      ...
      function (job, callback) {
        ...
        var usr_id_enabled : string = Meteor.call('validateWaiterEnabled', data_detail.restaurant_id);
        if( usr_id_enabled ){
  ...
  Meteor.methods({
     ...
     */
    validateWaiterEnabled ( _restaurant : string) : string {
      console.log('---------------------------------------------------------------------------------------------------');
      ...
      if(waiterEnabled.count() > 0){
        ...
        console.log('while -> position : ' + position);
        console.log('while -> _randomLast : ' + _randomLast);
