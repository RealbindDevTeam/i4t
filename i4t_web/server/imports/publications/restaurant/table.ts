...
Meteor.publish('getAllTables', function () {
    return Tables.find({ is_active: true });
});
...
Meteor.publish('getTablesByRestaurant', function (_restaurantId: string) {
    ...
    return Tables.find({ restaurantId: _restaurantId, is_active: true });
});
...
Meteor.publish('getTablesByRestaurantWork', function (_userId: string) {
    ...
    if (_lUserDetail) {
        return Tables.find({ restaurantId: _lUserDetail.restaurant_work, is_active: true });
    } else {
        return;
    }
});
